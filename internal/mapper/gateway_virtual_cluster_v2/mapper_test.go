...
func TestVirtualClusterV2KafkaModelMapping(t *testing.T) {
	ctx := context.Background()

	jsonVirtualClusterV2Resource := []byte(test.TestAccTestdata(t, "gateway/virtual_cluster_v2/api.json"))
	...
	assert.Equal(t, "vcluster1", ctlResource.Name)
	...
	assert.Equal(t, "vcluster1", internal.Metadata.Name)
	...
	// fmt.Println(fmt.Sprintf("tfmodel: %#v", tfModel.Spec.ClientProperties))

	assert.Equal(t, types.StringValue("vcluster1"), tfModel.Name)
	// assert.Equal(t, basetypes.BoolTypefalse, tfModel.Spec.AclEnabled)
	...
	assert.Equal(t, "vcluster1", internal2.Metadata.Name)
