...
func TestKafkaConnectV2ModelMapping(t *testing.T) {
	...
	jsonKafkaConnectV2Resource := []byte(test.TestAccTestdata(t, "kafka_connect_v2_api.json"))
	...
	internal, err := model.NewKafkaConnectResourceFromClientResource(ctlResource)
	if err != nil {
		t.Fatal(err)
		return
	}
